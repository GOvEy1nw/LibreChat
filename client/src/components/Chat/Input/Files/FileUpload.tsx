...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    reader.onload = (e) => {
      ...
      if (validator) {
        setStatus('success');
        setStatusColor('text-sky-500 dark:text-sky-500');
      }
  ...
  return (
    <label
      ...
      className={cn(
        'mr-1 flex h-auto cursor-pointer items-center rounded bg-transparent px-2 py-1 text-xs font-normal transition-colors hover:bg-gray-100 hover:text-sky-600 dark:bg-transparent dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-sky-500',
        statusColor,
        containerClassName,
      )}
    >
